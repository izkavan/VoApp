...
export function initializeScriptImportModal() {
  ...
  toggleSwitch = document.getElementById(
    "script-import-toggle",
  ) as HTMLInputElement;
  trimWordCheckbox = document.getElementById(
    "script-import-trim-word",
  ) as HTMLInputElement;
  addCharBtn = document.getElementById(
    "script-import-add-char",
  ) as HTMLButtonElement;
  addSceneBtn = document.getElementById(
    "script-import-add-scene",
  ) as HTMLButtonElement;
  addTitleBtn = document.getElementById(
    "script-import-add-title",
  ) as HTMLButtonElement;
  resetBtn = document.getElementById(
    "script-import-reset-btn",
  ) as HTMLButtonElement;
  projectSelect = document.getElementById(
    "script-import-project",
  ) as HTMLSelectElement;
  characterSelect = document.getElementById(
    "script-import-character",
  ) as HTMLSelectElement;
  listContainer = document.getElementById("script-import-list") as HTMLElement;
  textareaContainer = document.getElementById(
    "script-import-textarea",
  ) as HTMLElement;
  cancelBtn = document.getElementById(
    "script-import-cancel",
  ) as HTMLButtonElement;
  ...
  addCharBtn.addEventListener("click", () =>
    assignSelection("line", characterSelect.value || "none"),
  );
  addSceneBtn.addEventListener("click", () => assignSelection("line", "scene"));
  addTitleBtn.addEventListener("click", () =>
    assignSelection("title", "scene"),
  );

  resetBtn.addEventListener("click", handleReset);

  saveBtn.addEventListener("click", () => {
    const unassignedSpans = textareaContainer.querySelectorAll(
      "span.import-highlight:not(.import-assigned)",
    );
    ...
    if (hasUnassigned) {
      if (
        !confirm(
          "There is still unassigned text in the script. Are you sure you want to save?",
        )
      ) {
        return;
      }
...
export function openScriptImportModal(
  projects: Project[],
  characters: Character[],
  textContent: string,
  onSave: (lines: AssignedLine[]) => void,
) {
...
function handleKeyDown(e: KeyboardEvent) {
  ...
  const isInputFocused =
    targetTag === "input" || targetTag === "select" || targetTag === "textarea";
...
function handleProjectChange() {
  ...
  characterSelect.innerHTML =
    ...
    generateCharacterOptionsHTML(charactersData, pId);
  updateButtonStates();
}
...
function handleSegmentClick(e: MouseEvent) {
  ...
  if (lockedElementId === target.dataset.id) {
    ...
    if (lockedElementId) {
      const oldLocked = textareaContainer.querySelector(
        `span[data-id="${lockedElementId}"]`,
      );
...
function updateButtonStates() {
  ...
  const hasLock =
    lockedElementId !== null && lockedElementId.startsWith("seg-");
...
function assignSelection(type: "line" | "title", characterId: string) {
  ...
  const lockedSpan = textareaContainer.querySelector(
    `span[data-id="${lockedElementId}"]`,
  ) as HTMLElement;
  ...
  assignedLines.forEach((line) => {
    ...
    if (line.type === "title") {
      ...
      if (line.characterId !== "none") {
        const c = charactersData.find(
          (c) => c.id.toString() === line.characterId,
        );
    ...
    item.addEventListener("click", () => {
      ...
      if (lockedElementId === line.id) {
        ...
        if (
          ...
          line.characterId !== "none"
        ) {
          const char = charactersData.find(
            (c) => c.id.toString() === line.characterId,
          );
