...
export class TableReadLine extends HTMLElement {
    ...
    connectedCallback() {
        this.addEventListener('click', () => {
            this.dispatchEvent(new CustomEvent('lineSelected', { detail: this.index }));
        });
        this.render();
    }
    ...
    private render() {
        ...
        if (this.line.takes.length > 0) {
            indicatorClass = this.line.preferredTakeId ? 'indicator-green' : 'indicator-yellow';
        }

        const isSceneOrTitle = !this.line.characterName || this.line.characterName.toLowerCase() === 'scene' || this.line.characterName.toLowerCase() === 'title';
        ...
        const charSpanHtml = this.line.characterName 
            ? html`<span style="font-weight: bold; margin-right: 8px;">[${this.line.characterName}]</span>`
            : '';

        const textPreview = this.line.text.length > 50 ? this.line.text.substring(0, 50) + '...' : this.line.text;
...
export class TableReadTake extends HTMLElement {
    ...
    private render() {
        ...
        const audioPlayer = html`<audio controls controlsList="nodownload" src="${this.take.audioData}"></audio>`;
        ...
        if (prefBtn) {
            prefBtn.addEventListener('click', () => {
                this.dispatchEvent(new CustomEvent('togglePreferred', { detail: this.take.id }));
            });
        }
    }
}
