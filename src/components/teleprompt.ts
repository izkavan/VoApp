...
export function initializeTeleprompter(projects: Project[]) {
  ...
  const fontMinus = document.getElementById(
    "teleprompt-font-minus",
  ) as HTMLButtonElement;
  const fontPlus = document.getElementById(
    "teleprompt-font-plus",
  ) as HTMLButtonElement;
  const fontLabel = document.getElementById(
    "teleprompt-font-label",
  ) as HTMLSpanElement;
  const spacingMinus = document.getElementById(
    "teleprompt-spacing-minus",
  ) as HTMLButtonElement;
  const spacingPlus = document.getElementById(
    "teleprompt-spacing-plus",
  ) as HTMLButtonElement;
  const spacingLabel = document.getElementById(
    "teleprompt-spacing-label",
  ) as HTMLSpanElement;
  ...
  const readToggle = document.getElementById(
    "teleprompt-read-toggle",
  ) as HTMLInputElement;
  const recordButton = document.getElementById(
    "teleprompt-record-button",
  ) as HTMLButtonElement;

  const takesList = document.getElementById(
    "teleprompt-takes-list",
  ) as HTMLUListElement;
  const saveButton = document.getElementById(
    "teleprompt-save-session",
  ) as HTMLButtonElement;
  const clearButton = document.getElementById(
    "teleprompt-clear-session",
  ) as HTMLButtonElement;
  ...
  const renderTOC = () => {
    tocList.innerHTML = sessionFiles
      .map(
        (f) => `
  ...
  displayDiv.addEventListener("click", (e) => {
    ...
    if (sentenceEl) {
      ...
      if (activeFile.highlightedSentences.includes(idx)) {
        activeFile.highlightedSentences =
          activeFile.highlightedSentences.filter((i) => i !== idx);
        sentenceEl.classList.remove("highlighted");
      } else {
        activeFile.highlightedSentences = [];
        displayDiv
          .querySelectorAll(".sentence.highlighted")
  ...
  const renderText = () => {
    ...
    let globalIndex = 0;
    let sentenceWrapped = activeFile.content
      .split(/\r?\n/)
      .map((line) => {
        if (!line) return "<br>";
        return (line.match(/[^.!?]+[.!?]*\s*/g) || [line])
          .map((s) => {
            ...
            const isHighlighted = activeFile.highlightedSentences.includes(idx)
              ? " highlighted"
              : "";
    ...
    const finalHtml = highlightDictionaryWords(
      sentenceWrapped,
      currentDictionary,
    );
  ...
  const scrollToHighlight = () => {
    ...
    if (activeFile.highlightedSentences.length > 0) {
      ...
      const firstHighlightEl = displayDiv.querySelector(
        `.sentence[data-index="${firstHighlightIdx}"]`,
      ) as HTMLElement;
  ...
  spacingMinus.addEventListener("click", () => {
    if (lineHeightRatio > 1.0) {
      lineHeightRatio = Math.max(1.0, lineHeightRatio - 0.1);
      updateLineSpacing();
    }
  });
  spacingPlus.addEventListener("click", () => {
    if (lineHeightRatio < 4.0) {
      lineHeightRatio = Math.min(4.0, lineHeightRatio + 0.1);
      updateLineSpacing();
    }
  });

  colorPicker.addEventListener("input", (e) => {
    displayDiv.style.color = (e.target as HTMLInputElement).value;
    saveState();
  });
  bgColorPicker.addEventListener("input", (e) => {
    displayDiv.style.backgroundColor = (e.target as HTMLInputElement).value;
    saveState();
  });
  ...
  const renderTakes = async () => {
    ...
    await Promise.all(
      activeFile.takes.map(async (take) => {
    ...
    takesList.innerHTML = activeFile.takes
      .map(
        (take, index) => `
    ...
    takesList.querySelectorAll(".tp-delete-take").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".tp-take-item")
    ...
    takesList.querySelectorAll(".tp-download-take").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".tp-take-item")
    ...
    takesList.querySelectorAll(".tp-edit-take").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".tp-take-item")
    ...
    takesList.querySelectorAll(".tp-title-input").forEach((input) =>
    ...
    takesList.querySelectorAll(".take-note-input").forEach((input) =>
      input.addEventListener("input", (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".take-item")
    ...
    document.querySelectorAll("#teleprompt-takes-list .star").forEach((star) =>
  ...
  recordButton.addEventListener("click", async () => {
    ...
    try {
      ...
      mediaRecorder.addEventListener("stop", async () => {
        ...
        activeFile.takes.push({
          audioId,
          rating: 0,
          notes: "",
          title: `Take ${activeFile.takes.length + 1}`,
        });
        saveState();
        renderTakes();
  ...
  if (clearButton) {
    clearButton.addEventListener("click", () => {
      if (
        confirm(
          "Are you sure you want to clear the session? All unsaved takes will be lost.",
        )
      ) {
        sessionFiles = [];
        activeFileId = null;
