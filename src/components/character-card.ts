...
export class CharacterCard extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.addEventListener('click', (e) => {
            if ((e.target as HTMLElement).classList.contains('play-reference-btn')) return;
            if (this.character) {
    ...
    render() {
        ...
        if (this.character.artwork) {
            artworkHtml = html`<img src="${this.character.artwork}" class="character-card-artwork">`;
        }
        
        let odditiesHtml = '';
        if (this.character.characterOddities) {
            odditiesHtml = html`<p class="character-oddities" style="font-size: 0.85em; font-style: italic; margin-top: 5px;"><strong>Oddities:</strong> ${this.character.characterOddities}</p>`;
        }

        let audioButtonHtml = '';
        if (this.character.voice_sample) {
            audioButtonHtml = html`<button class="play-reference-btn" style="margin-top: 10px; padding: 4px 8px; font-size: 0.9em;">Play Reference</button>`;
        }
