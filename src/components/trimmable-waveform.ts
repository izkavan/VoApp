export class TrimmableWaveform {
    ...
    public setBuffer(buffer: AudioBuffer, initialStart?: number, initialEnd?: number) {
        this.buffer = buffer;
    ...
    private setupEvents() {
        ...
        window.addEventListener('mousemove', (e) => {
            ...
            if (this.isDraggingLeft) {
                ...
                this.endTime = Math.min(this.duration, Math.max(time, this.startTime + 0.1));
            }
    ...
    public draw(primaryColor: string = '#007bff') {
        ...
        // Right dark region
        if (endX < this.canvas.width) this.ctx.fillRect(endX, 0, this.canvas.width - endX, this.canvas.height);
