...
export function initializeDictionaryHighlighter() {
  ...
  document.body.addEventListener("mouseover", (e) => {
    ...
    if (
      target &&
      target.classList &&
      target.classList.contains("dict-highlight")
    ) {
      showTooltip(target);
...
export function highlightDictionaryWords(
  text: string,
  ...
  return parts
    .map((part) => {
      ...
      return part.replace(wordPattern, (match) => {
        ...
        if (entry) {
          const audioAttr = entry.audioData
            ? `data-audio="${entry.audioData}"`
            : "";
          const phoneticAttr = entry.phonetic
            ? `data-phonetic="${entry.phonetic.replace(/"/g, "&quot;")}"`
            : "";
