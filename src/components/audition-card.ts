...
export class AuditionCard extends HTMLElement {
  ...
  private render() {
    ...
    for (let i = 1; i <= 5; i++) {
      const isActive =
        this.audition.rating && i <= this.audition.rating ? "active" : "";
      starsHtml += html`<span
        class="vp-auditions-star ${isActive}"
    ...
    this.querySelectorAll(".vp-auditions-star").forEach((star) => {
      star.addEventListener("click", (e) => {
        const rating = parseInt(
          (e.target as HTMLElement).dataset.rating || "0",
        );
        if (this.audition) {
          this.audition.rating = rating;
          this.dispatchEvent(
            new CustomEvent("ratingChanged", { detail: rating }),
          );
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener("click", () => {
        if (
          ...
          this.dispatchEvent(
            new CustomEvent("deleteAudition", { detail: this.audition }),
          );
        }
      });
    }
