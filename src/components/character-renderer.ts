...
export function renderCharacterList(
  ...
  projects.forEach((project) => {
    const projectSection = createProjectSection(
      project,
      onCharacterDrop,
      openProjectModalCallback,
      openDictionaryModalCallback,
    );
    characterListElement.appendChild(projectSection);

    const projectCharacters = characterData.filter(
      (c) => c.projectId === project.id,
    );
    const characterCardsGrid = projectSection.querySelector(
  ...
  const unassignedSection = createProjectSection(
    unassignedProject,
    onCharacterDrop,
    openProjectModalCallback,
    openDictionaryModalCallback,
  ...
  const unassignedCardsGrid = unassignedSection.querySelector(
    ".character-cards-grid",
  );
