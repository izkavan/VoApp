...
export function saveProject() {
  ...
  if (nameExists) {
    if (
      !confirm(
        "A project with this name already exists, and could cause confusion. Do you wish to proceed?",
      )
    ) {
      return;
    }
...
async function exportProject() {
  ...
  if (charactersFolder) {
    for (const character of projectCharacters) {
      const charFolder = charactersFolder.folder(
        character.name.replace(/[^a-zA-Z0-9]/g, "_"),
      );
      if (charFolder) {
        ...
        if (character.artwork) {
          ...
          charFolder.file(
            character.artworkFilename || "artwork.png",
            artworkData,
            { base64: true },
          );
        }
