...
export class AudioEditorCanvas {
    ...
    private clearSelection() {
        ...
        if (this.onSelectionChange) this.onSelectionChange(this.selectedType, this.selectedIndex);
    }
    ...
    private getChunkAtTime(time: number): { index: number, localTime: number } | null {
        let currentStart = 0;
    ...
    private handleDoubleClick(e: MouseEvent) {
        ...
        this.chunks.splice(hit.index, 1, 
    ...
    private copyBuffer(buffer: AudioBuffer, startTime: number, endTime: number): AudioBuffer {
        const sampleRate = buffer.sampleRate;
    ...
    public render() {
        this.totalDuration = this.chunks.reduce((sum, c) => sum + c.buffer.duration, 0);
        ...
        for (let i = 0; i < this.chunks.length; i++) {
            ...
            const chunkWidth = (chunk.buffer.duration / this.totalDuration) * this.canvas.width;
    ...
    private drawWaveformChunk(chunk: AudioChunk, xOffset: number, width: number) {
        ...
        for (let i = 0; i < width; i++) {
            ...
            this.ctx.fillRect(xOffset + i, Math.min(yMin, yMax), 1, Math.max(1, Math.abs(yMax - yMin)));
        }
    }
}
