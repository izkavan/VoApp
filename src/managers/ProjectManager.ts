...
export class AppProjectManager {
     ...
     * moves all associated characters to an "Unassigned" status.
     * @param id The ID of the project to delete.
     */
    deleteProject(id: number) {
        if (confirm('Are you sure you want to delete this project? All associated characters will be safely moved to "Unassigned".')) {
    ...
    async importProject(zipFile: File) {
        try {
            ...
            if (projects.some(p => p.name === projectData.name)) {
                if (!confirm(`A project with the name "${projectData.name}" already exists. Proceed with importing?`)) {
                    return;
                }
            }
