...
export class AppBackupService {
  ...
  async restoreFullBackup(
    ...
    const restoredSettings = await ZipService.readJsonFile<SystemSettings>(
      zip,
      "settings.json",
      currentSettings,
    );
    const restoredCharacters = await ZipService.readJsonFile<Character[]>(
      zip,
      "characters.json",
      [],
    );
    const restoredProjects = await ZipService.readJsonFile<Project[]>(
      zip,
      "projects.json",
      [],
    );
    const restoredAuditions = await ZipService.readJsonFile<Audition[]>(
      zip,
      "auditions.json",
      [],
    );
    const memosMetadata = await ZipService.readJsonFile<any[]>(
      zip,
      "memos.json",
      [],
    );
    ...
    for (const meta of memosMetadata) {
      const memoFile =
        zip.file(`memos/${meta.id}.webm`) || zip.file(`memos/${meta.id}.wav`);
      if (memoFile) {
        ...
        try {
          ...
          console.error("Failed to save restored voice memo:", error);
          import("./EventBus.js").then((module) => {
            module.EventBus.emit("notify", {
              message: "Failed to restore a voice memo. Storage may be full.",
              type: "error",
            });
          });
        }
