...
function openEffectModal(effect: Effect | null) {
    ...
    if (effect) {
        ...
        tagsInput.value = effect.tags.join(', ');
...
async function saveModalChanges() {
    ...
    const tagsStr = (document.getElementById('effect-modal-tags-input') as HTMLInputElement).value;
...
async function downloadSelectedEffects() {
    ...
    try {
        ...
        a.href = url;
        a.download = `Effects_Export_${new Date().getTime()}.zip`;
