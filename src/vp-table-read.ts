...
function renderLinesList() {
    ...
    tableLines.forEach((line, index) => {
        ...
        lineEl.className = 'table-read-line-item';
        if (selectedLineIndex === index) {
            lineEl.classList.add('selected');
        }
        if (line.isUnmatched) {
            lineEl.classList.add('unmatched-line');
            lineEl.title = 'Unable to match script line.';
        }
        ...
        if (line.takes.length > 0) {
            indicatorClass = line.preferredTakeId ? 'indicator-green' : 'indicator-yellow';
        }

        const isSceneOrTitle = !line.characterName || line.characterName.toLowerCase() === 'scene' || line.characterName.toLowerCase() === 'title';
        if (isSceneOrTitle) {
            indicator.style.visibility = 'hidden';
            indicator.style.width = '12px'; // keep spacing
        } else {
            indicator.className = `table-read-indicator ${indicatorClass}`;
        }
        
        const textCont = document.createElement('div');
        textCont.className = 'table-read-line-text';
        
        if (line.characterName) {
            const charSpan = document.createElement('span');
            charSpan.style.fontWeight = 'bold';
            charSpan.style.marginRight = '8px';
            charSpan.textContent = `[${line.characterName}]`;
            textCont.appendChild(charSpan);
        }
        
        const txtNode = document.createTextNode(line.text.length > 50 ? line.text.substring(0, 50) + '...' : line.text);
        textCont.appendChild(txtNode);
        
        lineEl.appendChild(indicator);
        lineEl.appendChild(textCont);
...
function renderTakesList() {
    ...
    line.takes.forEach(take => {
        const takeEl = document.createElement('div');
        takeEl.className = 'table-read-take-card';
        if (line.preferredTakeId === take.id) {
            takeEl.classList.add('golden'); // Keeping CSS class name but we can rename it if we want
        }

        const info = document.createElement('div');
        info.innerHTML = `<strong>${take.title || 'Take'}</strong> <span style="color: var(--gray-500); font-size: 0.9em;">(from ${take.sourceZip})</span>`;
        ...
        // The controlsList attribute is not natively typed in all TS DOM lib versions without a cast
        audioPlayer.setAttribute('controlsList', 'nodownload');
        audioPlayer.src = take.audioData;
        audioPlayer.onplay = () => stopPlayback(); // stop any sequence playback if manual play
        
        const prefBtn = document.createElement('button');
        prefBtn.className = line.preferredTakeId === take.id ? 'primary' : 'secondary';
        prefBtn.textContent = line.preferredTakeId === take.id ? '★ Preferred' : 'Mark Preferred';
        prefBtn.onclick = () => {
            if (line.preferredTakeId === take.id) {
                line.preferredTakeId = null;
            } else {
                line.preferredTakeId = take.id;
            }
            renderUI();
        };
        ...
        takeEl.appendChild(info);
        takeEl.appendChild(controls);
