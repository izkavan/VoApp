...
export function initializeVPAuditionsView(
    ...
    projectSelect.addEventListener('change', (e) => {
        activeProjectId = (e.target as HTMLSelectElement).value;
        activeCharacter = '';
        renderCharacters();
        renderAuditions();
    });
...
function renderCharacters() {
    ...
    const filtered = activeProjectId 
        ? receivedAuditions.filter(a => a.project === activeProjectId)
        : receivedAuditions;
...
function renderAuditions() {
    ...
    if (activeProjectId) {
        filtered = filtered.filter(a => a.project === activeProjectId);
    }
    ...
    // Shuffle securely to make it truly blind, but keep a stable "Audition 1" index. 
    // For simplicity, we just sort by ID so the order is stable during review.
    filtered.sort((a, b) => a.id - b.id);

    filtered.forEach((aud, index) => {
        const card = document.createElement('div');
        card.className = 'vp-auditions-card';

        const header = document.createElement('div');
        header.className = 'vp-auditions-card-header';
        header.textContent = `Audition ${index + 1}`;

        const starsContainer = document.createElement('div');
        starsContainer.className = 'vp-auditions-stars';
        
        for (let i = 1; i <= 5; i++) {
            const star = document.createElement('span');
            star.className = 'vp-auditions-star';
            star.textContent = '★';
            if (aud.rating && i <= aud.rating) {
        ...
        commentsBox.className = 'vp-auditions-comments';
        commentsBox.placeholder = 'Leave notes here...';
        commentsBox.value = aud.comments || '';
        commentsBox.addEventListener('change', (e) => {
            aud.comments = (e.target as HTMLTextAreaElement).value;
            saveCallback(receivedAuditions);
        });
