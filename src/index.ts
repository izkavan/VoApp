...
import {
  ...
  openModal,
  closeModal,
} from "./components/character-modal.js";
import {
  initializeProjectModal,
  openProjectModal,
  closeProjectModal,
  saveProject,
} from "./components/project-modal.js";
import {
  initializeFilterSearch,
  filterCharacters,
  updateFilterData,
} from "./components/filter-search.js";
...
import {
  ...
  refreshLineReaderView,
} from "./views/line-reader.js";
import { initializeUtilityView } from "./views/utility-view.js";
import {
  initializeAuditionView,
  refreshAuditionView,
} from "./views/audition-view.js";
import {
  initializeTeleprompter,
  refreshTeleprompterProjects,
} from "./components/teleprompt.js";
import {
  initializeDungeonMasterView,
  refreshDungeonMasterView,
} from "./views/dungeon-master-view.js";
import {
  initializeCharacterNotesView,
  refreshCharacterNotesView,
} from "./views/character-notes-view.js";
import { initializeSettingsView } from "./views/settings-view.js";
import {
  initializeDictionaryModal,
  openDictionaryModal,
} from "./components/dictionary-modal.js";
import { initializeDictionaryHighlighter } from "./components/dictionary-highlighter.js";
import {
  initializeWarmupView,
  refreshWarmupView,
} from "./views/warmup-view.js";
import {
  initializeEffectLibrary,
  refreshEffectLibraryView,
} from "./views/effect-library.js";
import { initializeVoiceProductionFeedback } from "./views/voice-production/vp-feedback.js";
import {
  initializeScriptView,
  refreshScriptView,
} from "./views/voice-production/vp-script.js";
import {
  initializeSidesView,
  refreshSidesView,
} from "./views/voice-production/vp-sides.js";
import { initializeContrasterView } from "./views/voice-production/vp-contraster.js";
import {
  initializeVPAuditionsView,
  refreshVPAuditionsView,
} from "./views/voice-production/vp-auditions.js";
import {
  initializeTableReadView,
  setTableReadData,
} from "./views/voice-production/vp-table-read.js";
import { initializeAudioOverlay } from "./views/utilities/audio-overlay.js";
import {
  initializeVoiceMemos,
  refreshVoiceMemosProjects,
} from "./views/utility/voice-memos.js";
...
const filterTagsInput = document.getElementById(
  "filter-tags",
) as HTMLInputElement;
...
const exclusiveToggle = document.getElementById(
  "exclusive-toggle",
) as HTMLInputElement;
...
const projectLicensingInput = document.getElementById(
  "project-licensing",
) as HTMLInputElement;
...
// --- Dictionary Modal ---
const dictionaryModalElement = document.getElementById(
  "dictionary-modal",
) as HTMLElement;
const dictionaryModalContentElement = document.getElementById(
  "dictionary-modal-content",
) as HTMLElement;

export function initialRenderApp() {
...
EventBus.on("charactersUpdated", () => {
  const chars = DataStore.getCharacters();
  const projs = DataStore.getProjects();
  updateFilterData(chars, projs);
  filterCharacters();
  refreshDungeonMasterView(projs, chars);
  refreshCharacterNotesView(projs, chars);
  refreshScriptView(projs, chars);
  refreshSidesView(chars);
  refreshLineReaderView(projs, chars);
  refreshEffectLibraryView(projs, chars);
  refreshWarmupView(projs, chars);
  refreshAuditionView(chars);
  refreshVPAuditionsView(projs, chars);
});

EventBus.on("projectsUpdated", () => {
  const chars = DataStore.getCharacters();
  const projs = DataStore.getProjects();
  updateFilterData(chars, projs);
  refreshDungeonMasterView(projs, chars);
  ...
  refreshScriptView(projs, chars);
  refreshLineReaderView(projs, chars);
  refreshTeleprompterProjects(projs);
  refreshEffectLibraryView(projs, chars);
  refreshWarmupView(projs, chars);
  refreshVoiceMemosProjects(projs);
  refreshVPAuditionsView(projs, chars);
});

EventBus.on("auditionsUpdated", () => {
  refreshAuditionView(DataStore.getCharacters());
});

EventBus.on("receivedAuditionsUpdated", () => {
  const chars = DataStore.getCharacters();
  const projs = DataStore.getProjects();
  refreshVPAuditionsView(projs, chars);
});
...
EventBus.on("warmupsUpdated", () => {
  refreshWarmupView(DataStore.getProjects(), DataStore.getCharacters());
});

EventBus.on("userProfileUpdated", () => {
  loadMeViewData();
});
...
async function initApp() {
  ...
  initializeDictionaryModal(
    dictionaryModalElement,
    dictionaryModalContentElement,
  );
