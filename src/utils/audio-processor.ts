...
export async function compileDemoReelBuffer(
  ...
  for (const clip of clips) {
    if (clip.buffer.numberOfChannels > maxChannels)
      maxChannels = clip.buffer.numberOfChannels;
    if (clip.buffer.sampleRate > maxSampleRate)
      maxSampleRate = clip.buffer.sampleRate;
  }
...
export async function exportDemoReel(
  ...
  if (format === "wav") {
    ...
    return new Promise((resolve) => {
      if (onProgress)
        onProgress(
          `Encoding WebM (takes ${Math.ceil(combinedBuffer.duration)}s)...`,
        );
