...
import { saveImageBlob, getImageBlob, saveAudioBlob, getAudioBlob } from '../services/indexeddb.js';
        ...
        <div style="max-width: 900px; margin: 0 auto; display: flex; flex-direction: column; gap: 30px;">
            ...
            <div>
                ...
                <div id="me-job-types" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-top: 10px; background: var(--surface-color); padding: 15px; border-radius: 8px;">
                    ${['Commercials', 'Animation', 'Video Games', 'Dubbing/ADR', 'Audiobooks', 'E-Learning', 'Promos', 'Corporate/Explainer', 'IVR/Telephony', 'Other']
                        .map(job => `<label><input type="checkbox" value="${job}" class="me-job-type-cb"> ${job}</label>`).join('')}
                </div>
...
function renderCustomLinks() {
    ...
    container.innerHTML = customLinksData.map((link, i) => `
        <div style="display: flex; gap: 10px; align-items: center;">
...
function populateRoleLookup() {
    ...
    const characters = [...DataStore.getCharacters()].sort((a, b) => a.name.localeCompare(b.name));
    
    lookup.innerHTML = '<option value="" disabled selected>Select character</option>';
...
export async function loadMeViewData() {
    ...
    (document.getElementById('me-first-name') as HTMLInputElement).value = profile.firstName || '';
    (document.getElementById('me-last-name') as HTMLInputElement).value = profile.lastName || '';
    (document.getElementById('me-email') as HTMLInputElement).value = profile.email || '';
    (document.getElementById('me-phone') as HTMLInputElement).value = profile.phone || '';
    (document.getElementById('me-address') as HTMLInputElement).value = profile.address || '';
    (document.getElementById('me-experience') as HTMLInputElement).value = profile.yearsOfExperience || '';
    
    const checkboxes = document.querySelectorAll('.me-job-type-cb') as NodeListOf<HTMLInputElement>;
    let hasOther = false;
    checkboxes.forEach(cb => {
        if (cb.value === 'Other') {
            const standardJobs = ['Commercials', 'Animation', 'Video Games', 'Dubbing/ADR', 'Audiobooks', 'E-Learning', 'Promos', 'Corporate/Explainer', 'IVR/Telephony', 'Other'];
            const customJobs = profile.preferredJobTypes?.filter(j => !standardJobs.includes(j)) || [];
            if (customJobs.length > 0 || profile.preferredJobTypes?.includes('Other')) {
                cb.checked = true;
                hasOther = true;
                (document.getElementById('me-job-type-other-input') as HTMLInputElement).value = customJobs.join(', ');
            }
        } else {
    ...
    if (profile.socialLinks) {
        (document.getElementById('me-twitter') as HTMLInputElement).value = profile.socialLinks.twitter || '';
        (document.getElementById('me-mastodon') as HTMLInputElement).value = profile.socialLinks.mastodon || '';
        (document.getElementById('me-bluesky') as HTMLInputElement).value = profile.socialLinks.bluesky || '';
        (document.getElementById('me-linkedin') as HTMLInputElement).value = profile.socialLinks.linkedin || '';
        (document.getElementById('me-site') as HTMLInputElement).value = profile.socialLinks.personalSite || '';
        
        customLinksData = Array.isArray(profile.socialLinks.custom) ? JSON.parse(JSON.stringify(profile.socialLinks.custom)) : [];
    } else {
        customLinksData = [];
    }
    renderCustomLinks();
            ...
            if (preview) {
                ...
                dlBtn.onclick = async (e) => {
                    ...
                    try {
                        ...
                        if (format === 'wav' && !drBlob.type.includes('wav') && !drBlob.type.startsWith('video')) {
                            const { convertWebMToWav } = await import('../utils/audio-utils.js');
                        ...
                        a.href = dlUrl;
                        a.download = drBlob.type.startsWith('video') ? (currentDemoReelFilename || 'demo_reel.webm') : `${filename}.${format}`;
                        a.click();
                    } catch (err) {
                        console.error(err);
...
function bindMeViewEvents() {
    document.getElementById('me-headshot-upload')?.addEventListener('change', (e: Event) => {
    ...
    document.getElementById('me-demoreel-upload')?.addEventListener('change', (e: Event) => {
        const file = (e.target as HTMLInputElement).files?.[0];
        if (file) {
    ...
    const saveHandler = async () => {
        const firstName = (document.getElementById('me-first-name') as HTMLInputElement).value;
        const lastName = (document.getElementById('me-last-name') as HTMLInputElement).value;
        const email = (document.getElementById('me-email') as HTMLInputElement).value;
        const phone = (document.getElementById('me-phone') as HTMLInputElement).value;
        const address = (document.getElementById('me-address') as HTMLInputElement).value;
        const yearsOfExperience = (document.getElementById('me-experience') as HTMLInputElement).value;
        ...
        document.querySelectorAll('.me-job-type-cb:checked').forEach(cb => {
            ...
            if (val === 'Other') {
                const otherInput = (document.getElementById('me-job-type-other-input') as HTMLInputElement).value;
                if (otherInput.trim()) {
                    preferredJobTypes.push(...otherInput.split(',').map(s => s.trim()).filter(s => s));
                } else {
        ...
        customLinksData = customLinksData.filter(l => l.name.trim() !== '' || l.url.trim() !== '');

        const socialLinks = {
            twitter: (document.getElementById('me-twitter') as HTMLInputElement).value,
            mastodon: (document.getElementById('me-mastodon') as HTMLInputElement).value,
            bluesky: (document.getElementById('me-bluesky') as HTMLInputElement).value,
            linkedin: (document.getElementById('me-linkedin') as HTMLInputElement).value,
            personalSite: (document.getElementById('me-site') as HTMLInputElement).value,
