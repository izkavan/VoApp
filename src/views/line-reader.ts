...
// In-memory representation of a recording take for a specific line
...
/**
 * In-memory representation of a line in the script.
 * Holds references to all recorded takes and user-provided notes.
 */
...
export function refreshLineReaderView(
  newProjects: Project[],
...
/**
 * Initializes the Line Reader view, setting up event listeners, local storage session persistence,
 * and audio recording mechanics via the MediaRecorder API.
 * @param characters Global characters list for dropdown mapping
 * @param projects Global projects list for dropdown mapping
 * @param settings Global settings for export formats
 * @param openCharacterModal Callback to open character reference sheets
 */
export function initializeLineReader(
  ...
  const scriptVersionInput = document.getElementById(
  ...
  const saveActiveSession = () => {
    const linesToSave = Array.from(
      document
        .getElementById("line-container")
        ?.querySelectorAll(".line-entry") || [],
    ).map((line) => {
      const status: "read" | "omitted" | "normal" = line.classList.contains(
        "read",
      )
        ? "read"
        : line.classList.contains("omitted")
          ? "omitted"
          : "normal";
      const text =
        line.getAttribute("data-line-text") || line.textContent || "";
  ...
  const loadActiveSession = async () => {
    ...
    try {
      ...
      if (sessionData.scriptName)
        scriptNameInput.value = sessionData.scriptName;
      if (sessionData.scriptVersion)
        scriptVersionInput.value = sessionData.scriptVersion;
      if (sessionData.projectId && projectSelect)
        projectSelect.value = String(sessionData.projectId);
      ...
      (sessionData.lines || []).forEach((lineInfo: any) => {
        ...
        if (lineInfo.status === "read") {
          ...
          readLineDiv.addEventListener("click", () =>
            selectLine(readLineDiv, true),
          );
  ...
  const getAvailableCharacters = () => {
    ...
    if (selectedProjectId !== undefined && !isNaN(selectedProjectId)) {
      return currentLineReaderCharacters.filter(
  ...
  const updateCharacterDropdowns = () => {
    if (filterSelect) {
      ...
      const selectedProjectId =
        projectSelect?.value && projectSelect.value !== ""
          ? Number(projectSelect.value)
          : undefined;
      filterSelect.innerHTML =
        `<option value="all">No Filter</option><option value="unassigned">Unassigned</option>` +
        generateCharacterOptionsHTML(
          currentLineReaderCharacters,
          selectedProjectId,
        );
      if (
        Array.from(filterSelect.options).some((o) => o.value === currentFilter)
      ) {
  ...
  projectSelect?.addEventListener("change", async () => {
    ...
    lineDetails.forEach((detail) => {
      if (
        ...
        !availableIds.has(detail.characterId)
      ) {
        detail.characterId = undefined;
      }
    });
    ...
    if (selectedReadLine) {
      renderDetails(
        selectedReadLine.getAttribute("data-line-text") ||
          selectedReadLine.textContent ||
          "",
      );
    }
    updateLineContainerUI();
  ...
  const updateLineContainerUI = () => {
    ...
    [lineContainer, readContainer].forEach((container) => {
      ...
      Array.from(container.children).forEach((child) => {
        ...
        const lineText =
          lineDiv.getAttribute("data-line-text") || lineDiv.textContent || "";
        ...
        if (lineDiv.classList.contains("read")) {
          ...
          if (state === "Done")
            stateDot = '<span class="status-dot dot-done"></span>';
          else if (state === "In Progress")
            stateDot = '<span class="status-dot dot-in-progress"></span>';
          else stateDot = '<span class="status-dot dot-not-started"></span>';

          if (detail && detail.characterId) {
            const char = currentLineReaderCharacters.find(
              (c) => c.id === detail.characterId,
            );
  ...
  const renderDetails = async (lineText: string) => {
    ...
    const associatedCharacter = currentLineReaderCharacters.find(
      (c) => c.id === details.characterId,
    );
    ...
    const char = details.characterId
      ? currentLineReaderCharacters.find((c) => c.id === details.characterId)
      : null;
    if (char) {
      readDetailsContainer.innerHTML += `
      ...
      setTimeout(() => {
        document
          .getElementById("read-view-profile-btn")
          ?.addEventListener("click", () => openCharacterModal(char));
      }, 0);
    }
    ...
    document
      ...
      ?.addEventListener("input", (e) => {
        details.lineName = (e.target as HTMLInputElement).value;
      });
    document
      .getElementById("record-take-button")
      ?.addEventListener("click", toggleRecording);
    document
      .getElementById("line-character-select")
    ...
    document
      .getElementById("line-character-art")
      ?.addEventListener("click", () => {
        if (associatedCharacter) openCharacterModal(associatedCharacter);
      });
    document.getElementById("line-notes")?.addEventListener("input", (e) => {
      details.notes = (e.target as HTMLTextAreaElement).value;
    });
    document
      .getElementById("line-state-select")
      ?.addEventListener("change", (e) => {
        details.state = (e.target as HTMLSelectElement).value as any;
        updateLineContainerUI();
        saveActiveSession();
      });
    document.querySelectorAll(".delete-take").forEach((btn) =>
    ...
    document.querySelectorAll(".edit-take").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".take-item")
        ...
        openEditAudioModal(blob, async (newBlob: Blob) => {
          ...
          const audioPlayer = (
            e.currentTarget as HTMLElement
          ).parentElement?.querySelector("audio");
          if (audioPlayer) {
    ...
    document.querySelectorAll(".download-take").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".take-item")
        ...
        const safeTitle = (take.title || `take_${index + 1}`)
          .replace(/[^a-z0-9]/gi, "_")
          .toLowerCase();
    ...
    document.querySelectorAll(".take-note-input").forEach((input) =>
    ...
    document.querySelectorAll(".take-title-input").forEach((input) =>
      input.addEventListener("input", (e) => {
        const index = Number(
          (e.currentTarget as HTMLElement)
            .closest(".take-item")
  ...
  const selectLine = (lineDiv: HTMLElement, isReadLine: boolean) => {
    ...
    const setter = (val: HTMLElement | null) =>
      isReadLine ? (selectedReadLine = val) : (selectedLine = val);
  ...
  const toggleRecording = async () => {
    ...
    try {
      ...
      alert(
        "Could not start recording. Please ensure you have granted microphone permissions.",
      );
    }
  };
  ...
  const resetUI = async () => {
    ...
    [lineContainer, readContainer, readDetailsContainer].forEach(
      (c) => c && (c.innerHTML = ""),
    );
  ...
  const loadScriptFromZip = async (file: File) => {
    ...
    try {
      ...
      for (const savedDetail of scriptData.lineDetails) {
        ...
        lineDetails.set(savedDetail.text, {
          ...savedDetail,
          takes: loadedTakes,
        });
      }
      ...
      alert(
        "Failed to load script. The file may be corrupted or in the wrong format.",
      );
    }
  };
  ...
  const processJsonScriptData = (data: any) => {
    ...
    if (data.projectId && projects.some((p) => p.id === data.projectId)) {
      ...
      alert(
        `Warning: The project linked to this script is not available. Character linking will be skipped.`,
      );
    }
    ...
    if (data.lines && Array.isArray(data.lines)) {
      data.lines.forEach((line: any, index: number) => {
        const isCharLine =
          line.type !== "title" &&
          line.characterId !== "scene" &&
          line.characterId !== null &&
          line.characterName !== "scene" &&
          line.characterName !== "title";
        ...
        if (isCharLine) {
          ...
          if (
            data.projectId &&
            projects.some((p) => p.id === data.projectId) &&
            line.characterId
          ) {
            const charIdNum = parseInt(line.characterId);
            const exists = characters.find(
              (c) => c.id === charIdNum && c.projectId === data.projectId,
            );
            if (exists) {
              resolvedCharId = charIdNum;
            } else if (line.characterName) {
              if (!missingCharacters.includes(line.characterName))
    ...
    if (missingCharacters.length > 0) {
      alert(
        `Characters ${missingCharacters.join(", ")} were not available for import linking.`,
      );
    }
  ...
  document
    ...
    ?.addEventListener("click", () => {
      if (
        confirm(
          "Are you sure you want to clear the session? Unsaved changes will be lost.",
        )
      ) {
        resetUI();
  ...
  readButton?.addEventListener("click", () => {
    if (selectedLine) {
      const lineText =
        selectedLine.getAttribute("data-line-text") ||
      ...
      if (selectedLine.classList.contains("read")) {
        ...
        if (readContainer) {
          const existing = Array.from(readContainer.children).find(
            (child) =>
              (child.getAttribute("data-line-text") || child.textContent) ===
              lineText,
          );
        ...
        if (readContainer) {
          if (
            !Array.from(readContainer.children).some(
              (child) =>
                (child.getAttribute("data-line-text") || child.textContent) ===
                lineText,
            )
            ...
            for (const child of Array.from(readContainer.children)) {
              const childLineText =
                child.getAttribute("data-line-text") || child.textContent || "";
              const childSourceLine = allSourceLines.find(
                (l) =>
                  (l.getAttribute("data-line-text") || l.textContent) ===
                  childLineText,
              );
              const childSourceIndex = childSourceLine
                ? allSourceLines.indexOf(childSourceLine)
                : -1;
  ...
  saveButton?.addEventListener("click", async () => {
    ...
    readLinesDetails.forEach((d) =>
      nameCounts.set(d.lineName, (nameCounts.get(d.lineName) || 0) + 1),
    );
    ...
    if (duplicates.length > 0) {
      alert(
        `Save failed. Duplicate line names found:\n- ${duplicates.join("\n- ")}`,
      );
      return;
    }
    ...
    for (const detail of readLinesDetails) {
      ...
      if (settings.scriptExportGrouping === "character") {
        ...
        const folderName = char
          ? char.name.replace(/[^a-zA-Z0-9]/g, "_")
          : "Unassigned";
      ...
      for (let i = 0; i < detail.takes.length; i++) {
        ...
        const takeIdentifier = take.title
          ? take.title.replace(/[^a-zA-Z0-9]/g, "_")
          : `${i + 1}`;
    ...
    const linesToSave = Array.from(
      lineContainer?.querySelectorAll(".line-entry") || [],
    ).map((line) => {
      const status: "read" | "omitted" | "normal" = line.classList.contains(
        "read",
      )
  ...
  resetButton?.addEventListener("click", () => {
    const confirmed = confirm(
      "Are you sure you want to reset? All line statuses, notes, and recorded audio takes for this script will be lost.",
    );
    if (confirmed) {
