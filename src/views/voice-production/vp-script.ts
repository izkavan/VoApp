...
export function initializeScriptView(
  ...
  versionInput = document.getElementById(
    "vp-script-version",
  ) as HTMLInputElement;
  projectSelect = document.getElementById(
    "vp-script-project-select",
  ) as HTMLSelectElement;
  dictionaryBtn = document.getElementById(
    "vp-script-dictionary-btn",
  ) as HTMLButtonElement;
  importBtn = document.getElementById(
    "vp-script-import-btn",
  ) as HTMLButtonElement;
  saveBtn = document.getElementById("vp-script-save-btn") as HTMLButtonElement;
  sendSidesBtn = document.getElementById(
    "vp-script-send-sides-btn",
  ) as HTMLButtonElement;
  sendLineReadBtn = document.getElementById(
    "vp-script-send-line-read-btn",
  ) as HTMLButtonElement;
  importInput = document.getElementById(
    "vp-script-import-input",
  ) as HTMLInputElement;
  ...
  addLineBtn = document.getElementById(
    "vp-script-add-line-btn",
  ) as HTMLButtonElement;
  addTitleBtn = document.getElementById(
    "vp-script-add-title-btn",
  ) as HTMLButtonElement;
  ...
  sendLineReadBtn?.addEventListener("click", () =>
    sendScriptToExternal("line-read"),
  );
  ...
  window.addEventListener("dictionaryUpdated", async (e: Event) => {
    ...
    if (
      ...
      parseInt(projectSelect.value) === customEvent.detail.projectId
    ) {
      currentDictionary = await getDictionaryEntries(
...
function getScriptPayload() {
  return {
    name:
      (document.getElementById("vp-script-name") as HTMLInputElement)?.value ||
      "Script",
    version:
      (document.getElementById("vp-script-version") as HTMLInputElement)
        ?.value || "1.0",
    projectId:
      projectSelect.value === "none" ? null : parseInt(projectSelect.value),
    lines: scriptLines.map((line) => {
      ...
      return {
        ...
        type: line.type,
        characterId:
          line.characterId === "scene" ? null : parseInt(line.characterId),
...
export function refreshScriptView(
  newProjects: Project[],
...
async function handleImport(e: Event) {
  ...
  try {
    if (file.name.endsWith(".txt")) {
      ...
      openScriptImportModal(
        ...
        (assignedLines) => {
          ...
          const firstCharLine = assignedLines.find(
            (l) => l.characterId !== "scene" && l.characterId !== "none",
          );
          if (firstCharLine) {
            const char = characters.find(
              (c) => c.id.toString() === firstCharLine.characterId,
            );
    ...
    if (file.name.endsWith(".zip")) {
      ...
      throw new Error(
        "Unsupported file format. Please upload a .txt, .json or .zip file.",
      );
    }
