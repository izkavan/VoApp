...
export function initializeContrasterView(
  ...
  boardContainer = document.getElementById(
    "vp-contraster-board",
  ) as HTMLElement;
  importInput = document.getElementById(
    "vp-contraster-import-input",
  ) as HTMLInputElement;
  importBtn = document.getElementById(
    "vp-contraster-import-btn",
  ) as HTMLButtonElement;
  playSeqBtn = document.getElementById(
    "vp-contraster-play-btn",
  ) as HTMLButtonElement;
  exportBtn = document.getElementById(
    "vp-contraster-export-btn",
  ) as HTMLButtonElement;
  resetBtn = document.getElementById(
    "vp-contraster-reset-btn",
  ) as HTMLButtonElement;
  ...
  resetBtn.addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to clear all loaded auditions from the Contraster? This action cannot be undone.",
      )
    ) {
      stopAudio();
      receivedAuditions = [];
...
function renderBoard() {
  ...
  if (receivedAuditions.length === 0) {
    boardContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); width: 100%; margin-top: 40px;">Upload auditions to begin casting</div>';
    return;
  }
...
async function handleExport() {
  ...
  a.href = url;
  const projName = castList[0]?.project
    ? castList[0].project.replace(/\s+/g, "_")
    : "Project";
