...
export function refreshVPAuditionsView(
  newProjects: Project[],
...
export function initializeVPAuditionsView(
  ...
  saveCallback = onSave;
  importInput = document.getElementById(
    "vp-auditions-import-input",
  ) as HTMLInputElement;
  importBtn = document.getElementById(
    "vp-auditions-import-btn",
  ) as HTMLButtonElement;
  manualBtn = document.getElementById(
    "vp-auditions-manual-btn",
  ) as HTMLButtonElement;
  resetBtn = document.getElementById(
    "vp-auditions-reset-btn",
  ) as HTMLButtonElement;
  finalizeBtn = document.getElementById(
    "vp-auditions-finalize-btn",
  ) as HTMLButtonElement;
  finalistCountInput = document.getElementById(
    "vp-auditions-finalist-count",
  ) as HTMLInputElement;
  charactersContainer = document.getElementById(
    "vp-auditions-characters",
  ) as HTMLElement;
  ...
  resetBtn.addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to clear all loaded auditions? This action cannot be undone.",
      )
    ) {
      receivedAuditions = [];
...
async function processZipImport(file: File) {
  ...
  try {
    ...
    if (loadedZip.files["Audition_Master.json"]) {
      const masterStr =
        await loadedZip.files["Audition_Master.json"].async("string");
      try {
...
function addOrUpdateAudition(data: any) {
  ...
  const existing = receivedAuditions.find(
    (a) =>
...
function renderCharacters() {
  ...
  if (filtered.length === 0) {
    charactersContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">Upload auditions to view characters</div>';
    return;
  }

  const uniqueChars = Array.from(
    new Set(filtered.map((a) => a.character).filter(Boolean)),
  );
...
function renderAuditions() {
  ...
  if (!activeCharacter) {
    listContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">Select a character to review auditions</div>';
    return;
  }

  let filtered = receivedAuditions.filter(
    (a) => a.character === activeCharacter,
  );

  if (filtered.length === 0) {
    listContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">No auditions found for this character.</div>';
    return;
  }
  ...
  filtered.forEach((aud, index) => {
    ...
    card.addEventListener("ratingChanged", () =>
      saveCallback(receivedAuditions),
    );
    card.addEventListener("commentChanged", () =>
      saveCallback(receivedAuditions),
    );
    card.addEventListener("deleteAudition", (e: CustomEvent) => {
      const auditionToDelete = e.detail;
      receivedAuditions = receivedAuditions.filter(
        (a) => a.id !== auditionToDelete.id,
      );
      saveCallback(receivedAuditions);

      // Re-render
      const wasLastForCharacter =
        receivedAuditions.filter((a) => a.character === activeCharacter)
          .length === 0;
      ...
      if (charFolder) {
        ...
        if (actorFolder) {
          ...
          if (aud.audioData) {
            ...
            if (base64Data)
              actorFolder.file(audioFileName, base64Data, { base64: true });
          }
        }
      }
          ...
          if (tcCharFolder) {
            ...
            if (tcActorFolder) {
              ...
              if (aud.audioData) {
                const base64Data = aud.audioData.split(",")[1];
                if (base64Data)
                  tcActorFolder.file(audioFileName, base64Data, {
                    base64: true,
                  });
              }
            }
...
function openManualAuditionModal() {
  ...
  (
    document.getElementById("vp-manual-availability") as HTMLInputElement
  ).value = "";
  (document.getElementById("vp-manual-address") as HTMLInputElement).value = "";

  const audioInput = document.getElementById(
    "vp-manual-audio-upload",
  ) as HTMLInputElement;
  const audioStatus = document.getElementById(
    "vp-manual-audio-status",
  ) as HTMLSpanElement;
  audioInput.value = "";
  ...
  saveBtn!.onclick = () => {
    ...
    const characterId = (
      document.getElementById("vp-manual-character") as HTMLSelectElement
    ).value;
    const character =
      characters.find((c) => c.id.toString() === characterId)?.name ||
      characterId;
    const fName = (
      document.getElementById("vp-manual-fname") as HTMLInputElement
    ).value.trim();
    const lName = (
      document.getElementById("vp-manual-lname") as HTMLInputElement
    ).value.trim();
    ...
    const newAud: ReceivedAudition = {
      ...
      actorLastName: lName,
      actorEmail: (
        document.getElementById("vp-manual-email") as HTMLInputElement
      ).value,
      actorPhone: (
        document.getElementById("vp-manual-phone") as HTMLInputElement
      ).value,
      actorRate: (document.getElementById("vp-manual-rate") as HTMLInputElement)
        .value,
      actorAvailability: (
        document.getElementById("vp-manual-availability") as HTMLInputElement
      ).value,
      actorAddress: (
        document.getElementById("vp-manual-address") as HTMLInputElement
      ).value,
