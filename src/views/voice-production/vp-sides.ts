...
export function initializeSidesView(
  ...
  importInput = document.getElementById(
    "vp-sides-import-input",
  ) as HTMLInputElement;
  importBtn = document.getElementById(
    "vp-sides-import-btn",
  ) as HTMLButtonElement;
  exportFilenameInput = document.getElementById(
    "vp-sides-export-filename",
  ) as HTMLInputElement;
  exportBtn = document.getElementById(
    "vp-sides-export-btn",
  ) as HTMLButtonElement;
  includeContextToggle = document.getElementById(
    "vp-sides-include-context",
  ) as HTMLInputElement;
  ...
  loadScriptIntoSides = (data: any) => {
    ...
    if (exportFilenameInput) {
      ...
      const safeName = (scriptData?.name || "Script")
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
...
function renderCharacters() {
  ...
  if (!scriptData) {
    charactersContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">Upload a script to view characters</div>';
    return;
  }
  ...
  if (charNamesAndIds.size === 0) {
    charactersContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">No characters found in script.</div>';
    return;
  }
  ...
  charNamesAndIds.forEach((id, name) => {
    ...
    if (!artUrl) {
      ...
      if (charObjByName && charObjByName.artwork)
        artUrl = charObjByName.artwork;
    }
    if (artUrl) {
      img.src = artUrl;
    } else {
...
function renderLines() {
  ...
  if (!scriptData) {
    linesContainer.innerHTML =
      '<div style="text-align: center; color: var(--gray-500); margin-top: 20px;">Upload a script to view lines</div>';
    return;
  }
  ...
  scriptData.lines.forEach((line, index) => {
    ...
    if (line.characterId === "scene" || line.characterName === "scene")
...
function updateLinesFading() {
  ...
  Array.from(linesContainer.children).forEach((child) => {
    ...
    const isSceneOrTitle =
      charName === "scene" || charName === "title" || !charName;
