...
export function initializeTableReadView(openModalCb: (character: any) => void) {
  ...
  const importMasterBtn = document.getElementById(
    "vp-table-read-import-master-btn",
  ) as HTMLButtonElement;
  const importMasterInput = document.getElementById(
    "vp-table-read-import-master-input",
  ) as HTMLInputElement;
  const importBtn = document.getElementById(
    "vp-table-read-import-btn",
  ) as HTMLButtonElement;
  const importInput = document.getElementById(
    "vp-table-read-import-input",
  ) as HTMLInputElement;
  const resetBtn = document.getElementById(
    "vp-table-read-reset-btn",
  ) as HTMLButtonElement;
  const playBtn = document.getElementById(
    "vp-table-read-play-btn",
  ) as HTMLButtonElement;
  const importStateBtn = document.getElementById(
    "vp-table-read-import-state-btn",
  ) as HTMLButtonElement;
  const importStateInput = document.getElementById(
    "vp-table-read-import-state-input",
  ) as HTMLInputElement;
  const saveBtn = document.getElementById(
    "vp-table-read-save-btn",
  ) as HTMLButtonElement;

  linesContainer = document.getElementById(
    "vp-table-read-lines-container",
  ) as HTMLElement;
  takesContainer = document.getElementById(
    "vp-table-read-takes-container",
  ) as HTMLElement;
  selectedLineHeader = document.getElementById(
    "vp-table-read-selected-line-header",
  ) as HTMLElement;
  ...
  resetBtn.addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to clear the Table Read? This will remove all imported scripts and takes.",
      )
    ) {
      tableLines = [];
  ...
  try {
    ...
    if (charContainer) {
      ...
      masterCharacters.forEach((charId) => {
        const charObj = globalCharacters.find(
...
async function processZipImport(file: File) {
  ...
  try {
    ...
    if (!scriptDetailsFile) {
      alert(
        `No script.json found in ${file.name}. Ensure you are importing a zip exported from the Line Reader.`,
      );
      return;
    }
    ...
    if (
      scriptData.name !== masterScriptName ||
      scriptData.version !== masterScriptVersion
    ) {
      const proceed = confirm(
        `Warning: The imported line reads (${scriptData.name} v${scriptData.version}) do not match the master script (${masterScriptName} v${masterScriptVersion}). Proceed anyway?`,
      );
      if (!proceed) return;
    }
    ...
    if (scriptData.lineDetails) {
      ...
      for (const detail of scriptData.lineDetails) {
        ...
        let matchedIndex = tableLines.findIndex(
          (l) => l.text === detail.text && !l.isUnmatched,
        );
        ...
        if (lineObj) {
          ...
          const insertIndex =
            lastMatchedIndex !== -1 ? lastMatchedIndex + 1 : tableLines.length;
...
function renderLinesList() {
  ...
  if (tableLines.length === 0) {
    linesContainer.innerHTML =
      '<div style="color: var(--gray-500); text-align: center; margin-top: 20px;">Import ZIP to load script lines.</div>';
    return;
  }
...
function renderTakesList() {
  ...
  if (selectedLineIndex === null || selectedLineIndex >= tableLines.length) {
    ...
    takesContainer.innerHTML =
      '<div style="color: var(--gray-500); text-align: center; margin-top: 20px;">Select a line to view available takes.</div>';
    return;
  }
  ...
  if (line.takes.length === 0) {
    takesContainer.innerHTML =
      '<div style="color: var(--gray-500); text-align: center; margin-top: 20px;">No takes found for this line.</div>';
    return;
  }
...
async function handleStateSave() {
  ...
  zip.file(
    "Raw Data/table_read_state.json",
  ...
  if (rawFolder) {
    for (const line of tableLines) {
      const charFolder = line.characterName
        ? rawFolder.folder(line.characterName)
        : rawFolder.folder("Unassigned");
      if (charFolder) {
    ...
    for (const line of tableLines) {
      if (line.preferredTakeId) {
        ...
        if (take) {
          ...
          const charName = (line.characterName || "Unassigned").replace(
            /[^a-zA-Z0-9]/g,
            "_",
          );
          const title = (take.title || "Take").replace(/[^a-zA-Z0-9]/g, "_");
          prefFolder.file(`${seqStr}_${charName}_${title}.${ext}`, base64Data, {
            base64: true,
          });
          seq++;
        }
...
async function handleStateImport(e: Event) {
  ...
  try {
    ...
    if (!stateFile) {
      alert(
        "No table_read_state.json found in the imported ZIP. Ensure this is a Table Read export.",
      );
      return;
    }
