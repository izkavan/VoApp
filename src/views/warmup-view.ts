...
function setupFilters() {
  ...
  ratingCheckboxes.forEach((cb) =>
    cb.addEventListener("change", renderWarmupGrid),
  );
...
function populateCharacterSelect(projectIdStr: string) {
  ...
  characterSelect.innerHTML =
    `<option value="none">-- Filter by Character --</option>` +
    generateCharacterOptionsHTML(currentCharacters, pId);
}
...
function renderWarmupGrid() {
  ...
  const searchStr = (
    document.getElementById("warmup-tag-search") as HTMLInputElement
  ).value.toLowerCase();

  // Get checked ratings
  const ratingCheckboxes = document.querySelectorAll(
    ".warmup-rating-cb:checked",
  ) as NodeListOf<HTMLInputElement>;
  const allowedRatings = Array.from(ratingCheckboxes).map((cb) =>
    parseInt(cb.value),
  );

  const characterSelectVal = (
    document.getElementById("warmup-character-select") as HTMLSelectElement
  ).value;
  const projectSelectVal = (
    document.getElementById("warmup-project-select") as HTMLSelectElement
  ).value;
  ...
  currentWarmups.forEach((warmup) => {
    ...
    if (searchStr) {
      const matchesTag = warmup.tags.some((tag) =>
    ...
    if (characterSelectVal !== "none") {
      ...
      const projectCharIds = currentCharacters
        ...
        .map((c) => c.id);
      const hasProjectChar = warmup.characterIds.some((cId) =>
...
function openWarmupModal(warmup: Warmup | null) {
  ...
  if (warmup) {
    ...
    (
      document.getElementById("warmup-modal-title-input") as HTMLInputElement
    ).value = warmup.title;
    ...
    renderStars(0);
    document
      .getElementById("warmup-modal-rating")!
      .setAttribute("data-rating", "0");
...
function populateModalCharacterSelect() {
  ...
  sel.innerHTML =
    '<option value="none">-- Select Character to Add --</option>' +
    generateCharacterOptionsHTML(currentCharacters);
}
...
function setupAudioRecorder() {
  const recordBtn = document.getElementById(
    "warmup-record-button",
  ) as HTMLButtonElement;
  const statusText = document.getElementById(
    "warmup-record-status",
  ) as HTMLElement;
