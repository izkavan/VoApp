...
export function initializeVoiceMemos() {
  ...
  const importanceCheck = document.getElementById(
    "voice-memo-importance",
  ) as HTMLInputElement;
  const saveBtn = document.getElementById(
    "save-voice-memo-button",
  ...
  const activeTagsContainer = document.getElementById(
    "voice-memo-active-tags-container",
  );
  const tagSuggestions = document.getElementById("voice-memo-tag-suggestions");

  const searchInput = document.getElementById(
    "voice-memo-search-tags",
  ) as HTMLInputElement;
  const sortSelect = document.getElementById(
    "voice-memo-sort-select",
  ) as HTMLSelectElement;
  const importantFirstToggle = document.getElementById(
    "voice-memo-sort-important-first",
  ) as HTMLInputElement;
    ...
    if (mediaRecorder && mediaRecorder.state === "recording") {
      ...
      try {
        ...
        mediaRecorder.addEventListener("stop", () => {
          ...
          if (audioPreview) {
            audioPreview.innerHTML = html`<audio
              controls
              src="${audioUrl}"
            ></audio>`;
          }
  ...
  tagsInput?.addEventListener("input", () => {
    ...
    const matches = allExistingTags.filter(
      (t) => t.toLowerCase().includes(val) && !currentTags.includes(t),
    );
    matches.forEach((m) => {
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    const title =
      titleInput.value.trim() || `Memo ${new Date().toLocaleString()}`;
    const projectId = projectSelect.value
      ? parseInt(projectSelect.value, 10)
      : null;
    ...
    try {
      ...
      import("../../services/EventBus.js").then((module) => {
        module.EventBus.emit("notify", {
          message: "Failed to save voice memo. Storage may be full.",
          type: "error",
        });
      });
    }
  ...
  async function renderList() {
    ...
    memos.forEach((memo) => {
      ...
      const projName = memo.projectId
        ? projects.find((p) => p.id === memo.projectId)?.name ||
          "Unknown Project"
        : "Unassigned";
  ...
  deleteBtn?.addEventListener("click", async () => {
    ...
    if (
      confirm(`Are you sure you want to delete ${checkboxes.length} memo(s)?`)
    ) {
      try {
        const ids = Array.from(checkboxes).map((cb) =>
          parseInt(cb.dataset.id || "0", 10),
        ...
        import("../../services/EventBus.js").then((module) => {
          module.EventBus.emit("notify", {
            message: "Failed to delete voice memos.",
            type: "error",
          });
        });
      }
