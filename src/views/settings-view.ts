...
import { updateRecordTimerVisibility } from "../components/record-timer.js";
...
export function initializeSettingsView(
  ...
  const gearInput = document.getElementById(
    "settings-recording-gear",
  ) as HTMLTextAreaElement;
  const fontSelect = document.getElementById(
    "settings-system-font",
  ) as HTMLSelectElement;
  const fontWarning = document.getElementById(
    "opendyslexic-warning",
  ) as HTMLElement;

  // Feature Visibility Toggles
  const visViewVoiceActor = document.getElementById(
    "visibility-view-voice-actor",
  ) as HTMLInputElement;
  const visViewStoryteller = document.getElementById(
    "visibility-view-storyteller",
  ) as HTMLInputElement;
  const visViewVoiceProduction = document.getElementById(
    "visibility-view-voice-production",
  ) as HTMLInputElement;
  const visViewUtility = document.getElementById(
    "visibility-view-utility",
  ) as HTMLInputElement;
  const visShowRecordTimer = document.getElementById(
    "visibility-show-record-timer",
  ) as HTMLInputElement;
  const timerWarningInput = document.getElementById(
    "setting-timer-warning",
  ) as HTMLInputElement;
  const timerStopInput = document.getElementById(
    "setting-timer-stop",
  ) as HTMLInputElement;
  const visTabLineReader = document.getElementById(
    "visibility-tab-line-reader",
  ) as HTMLInputElement;
  const visTabTeleprompter = document.getElementById(
    "visibility-tab-teleprompter",
  ) as HTMLInputElement;
  const visTabAuditions = document.getElementById(
    "visibility-tab-auditions",
  ) as HTMLInputElement;
  const visTabEffectLibrary = document.getElementById(
    "visibility-tab-effect-library",
  ) as HTMLInputElement;
  const visTabVpFeedback = document.getElementById(
    "visibility-tab-vp-feedback",
  ) as HTMLInputElement;
  const visTabVpScript = document.getElementById(
    "visibility-tab-vp-script",
  ) as HTMLInputElement;
  const visTabVpSides = document.getElementById(
    "visibility-tab-vp-sides",
  ) as HTMLInputElement;
  const visTabVpAuditions = document.getElementById(
    "visibility-tab-vp-auditions",
  ) as HTMLInputElement;
  const visTabVpContraster = document.getElementById(
    "visibility-tab-vp-contraster",
  ) as HTMLInputElement;
  const visTabVpTableRead = document.getElementById(
    "visibility-tab-vp-table-read",
  ) as HTMLInputElement;
  const visTabDmSession = document.getElementById(
    "visibility-tab-dm-session",
  ) as HTMLInputElement;
  const visTabDmGenerator = document.getElementById(
    "visibility-tab-dm-generator",
  ) as HTMLInputElement;
  const visTabDmCharacterNotes = document.getElementById(
    "visibility-tab-dm-character-notes",
  ) as HTMLInputElement;
  const visTabAudioOverlay = document.getElementById(
    "visibility-tab-audio-overlay",
  ) as HTMLInputElement;
  const visTabWarmups = document.getElementById(
    "visibility-tab-warmups",
  ) as HTMLInputElement;
  const visTabVoiceMemos = document.getElementById(
    "visibility-tab-voice-memos",
  ) as HTMLInputElement;
  ...
  const backupBtn = document.getElementById(
    "settings-full-backup-btn",
  ) as HTMLButtonElement;
  const restoreFile = document.getElementById(
  ...
  if (timerWarningInput)
    timerWarningInput.value = (settings.timerWarningTime || 300).toString();
  if (timerStopInput)
    timerStopInput.value = (settings.timerStopTime || 600).toString();
  ...
  if (settings.featureVisibility) {
    if (visViewVoiceActor)
      visViewVoiceActor.checked =
        settings.featureVisibility.viewVoiceActor ?? true;
    if (visViewStoryteller)
      visViewStoryteller.checked =
        settings.featureVisibility.viewStoryteller ?? true;
    if (visViewVoiceProduction)
      visViewVoiceProduction.checked =
        settings.featureVisibility.viewVoiceProduction ?? true;
    if (visViewUtility)
      visViewUtility.checked = settings.featureVisibility.viewUtility ?? true;
    if (visShowRecordTimer)
      visShowRecordTimer.checked =
        settings.featureVisibility.showRecordTimer || false;
    if (visTabLineReader)
      visTabLineReader.checked =
        settings.featureVisibility.tabLineReader ?? true;
    if (visTabTeleprompter)
      visTabTeleprompter.checked =
        settings.featureVisibility.tabTeleprompter ?? true;
    if (visTabAuditions)
      visTabAuditions.checked = settings.featureVisibility.tabAuditions ?? true;
    if (visTabEffectLibrary)
      visTabEffectLibrary.checked =
        settings.featureVisibility.tabEffectLibrary ?? true;
    if (visTabVpFeedback)
      visTabVpFeedback.checked =
        settings.featureVisibility.tabVpFeedback ?? true;
    if (visTabVpScript)
      visTabVpScript.checked = settings.featureVisibility.tabVpScript ?? true;
    if (visTabVpSides)
      visTabVpSides.checked = settings.featureVisibility.tabVpSides ?? true;
    if (visTabVpAuditions)
      visTabVpAuditions.checked =
        settings.featureVisibility.tabVpAuditions ?? true;
    if (visTabVpContraster)
      visTabVpContraster.checked =
        settings.featureVisibility.tabVpContraster ?? true;
    if (visTabVpTableRead)
      visTabVpTableRead.checked =
        settings.featureVisibility.tabVpTableRead ?? true;
    if (visTabDmSession)
      visTabDmSession.checked = settings.featureVisibility.tabDmSession ?? true;
    if (visTabDmGenerator)
      visTabDmGenerator.checked =
        settings.featureVisibility.tabDmGenerator ?? true;
    if (visTabDmCharacterNotes)
      visTabDmCharacterNotes.checked =
        settings.featureVisibility.tabDmCharacterNotes ?? true;
    if (visTabAudioOverlay)
      visTabAudioOverlay.checked =
        settings.featureVisibility.tabAudioOverlay ?? true;
    if (visTabWarmups)
      visTabWarmups.checked = settings.featureVisibility.tabWarmups ?? true;
    if (visTabVoiceMemos)
      visTabVoiceMemos.checked =
        settings.featureVisibility.tabVoiceMemos ?? true;
  }
  ...
  clearCacheBtn?.addEventListener("click", () => {
    if (
      confirm(
        "WARNING: This is a complete system reset! All local storage and audio database data will be completely erased. Are you sure you want to proceed?",
      )
    ) {
      localStorage.clear();
  ...
  const triggerSave = () => {
    const newSettings: SystemSettings = {
      ...
      scriptExportGrouping:
        (groupingSelect?.value as "character" | "line") || "line",
  ...
  restoreBtn?.addEventListener("click", async () => {
    ...
    const confirmed = confirm(
      "WARNING: This will completely overwrite all your existing data, including characters, projects, scripts, and voice memos. Are you sure you want to proceed?",
    );
    if (!confirmed) return;
    ...
    try {
      ...
      alert(
        "Restore failed. The zip file may be corrupted or in an invalid format.",
      );
    } finally {
