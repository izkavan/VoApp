...
export function initializeAudioOverlay() {
  ...
  const handleSliderScroll = (
    ...
    let newVal =
      parseFloat(input.value) + (e.deltaY < 0 ? step * 10 : -step * 10);
  ...
  trackInput1.addEventListener("wheel", (e) =>
    handleSliderScroll(e, trackInput1, trackVal1, true),
  );
  trackInput2.addEventListener("wheel", (e) =>
    handleSliderScroll(e, trackInput2, trackVal2, false),
  );
...
function updateVolumes() {
  if (gain1)
    gain1.gain.value = muteInput1.checked ? 0 : parseFloat(volInput1.value);
  if (gain2)
    gain2.gain.value = muteInput2.checked ? 0 : parseFloat(volInput2.value);
}
