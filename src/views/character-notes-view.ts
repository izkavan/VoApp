...
export function refreshCharacterNotesView(
  newProjects?: Project[],
...
export function initializeCharacterNotesView(
  ...
  projectSelect = document.getElementById(
    "dm-notes-project-select",
  ) as HTMLSelectElement;
  characterSelect = document.getElementById(
    "dm-notes-character-select",
  ) as HTMLSelectElement;
  ...
  notesContainer = document.getElementById(
    "dm-notes-container",
  ) as HTMLDivElement;
  searchInput = document.getElementById("dm-notes-search") as HTMLInputElement;
  typeFilter = document.getElementById(
    "dm-notes-type-filter",
  ) as HTMLSelectElement;
  addTextBtn = document.getElementById(
    "dm-notes-add-text",
  ) as HTMLButtonElement;
  addSocialBtn = document.getElementById(
    "dm-notes-add-social",
  ) as HTMLButtonElement;
  addEventBtn = document.getElementById(
    "dm-notes-add-event",
  ) as HTMLButtonElement;
...
function renderList() {
  ...
  if (filtered.length === 0) {
    notesList.innerHTML =
      '<div style="padding:10px; color:#666; text-align:center;">No entries found.</div>';
    return;
  }
...
function renderEditor(entry: JournalEntry | null, isNew = false) {
  ...
  if (entry.type === "social") {
    ...
    const rVal = document.getElementById(
      "j-edit-rating-val",
    ) as HTMLSpanElement;
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    if (entry.type === "text") {
      ...
      social.npcName = (
        document.getElementById("j-edit-npcName") as HTMLInputElement
      ).value;
      entry.title = social.npcName || "Unnamed NPC";

      social.occupation = (
        document.getElementById("j-edit-occupation") as HTMLInputElement
      ).value;
      social.howTheyMet = (
        document.getElementById("j-edit-howTheyMet") as HTMLTextAreaElement
      ).value;
      social.currentOpinion = (
        document.getElementById("j-edit-currentOpinion") as HTMLTextAreaElement
      ).value;

      const newEventsInfluencing = (
        document.getElementById(
          "j-edit-eventsInfluencing",
        ) as HTMLTextAreaElement
      ).value;
      const newRating = parseInt(
        (document.getElementById("j-edit-rating") as HTMLInputElement).value,
        10,
      );
      ...
      if (isNew) {
        ...
        const reason = prompt(
          `You changed the rating from ${social.rating} to ${newRating}. What is your reasoning?`,
        );
        if (reason) {
          social.ratingHistory.push({
      ...
      const ev = entry as EventJournalEntry;
      ev.timeTookPlace = (
        document.getElementById("j-edit-timeTookPlace") as HTMLInputElement
      ).value;
      ev.description = (
        document.getElementById("j-edit-description") as HTMLTextAreaElement
      ).value;
      ev.openNotes = (
        document.getElementById("j-edit-openNotes") as HTMLTextAreaElement
      ).value;
    }
    ...
    try {
      ...
      import("../services/EventBus.js").then((module) => {
        module.EventBus.emit("notify", {
          message: "Failed to save journal entry. Storage may be full.",
          type: "error",
        });
      });
    }
...
async function handleExport() {
  ...
  const includeChar = confirm(
    "Do you want to include character details (e.g. voice sample, tags) in this export?",
  );
  ...
  input.onchange = async (e: Event) => {
    ...
    reader.onload = async (re) => {
      try {
        ...
        if (!targetCharId) {
          alert(
            "You have no character selected. Please create a new blank character for this journal import, then select them and import again.",
          );
        ...
        for (const entry of data.entries) {
          ...
          try {
            ...
            console.error("Failed to save imported journal entry:", error);
            import("../services/EventBus.js").then((module) => {
              module.EventBus.emit("notify", {
                message: "Failed to import entry. Storage may be full.",
                type: "error",
              });
            });
          }
