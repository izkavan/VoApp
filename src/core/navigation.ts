...
export function initializeNavigation() {
  ...
  const characterLibraryView = document.getElementById(
...
export function applyFeatureVisibility(settings: SystemSettings) {
  ...
  if (navVoiceActors)
    navVoiceActors.style.display = fv.viewVoiceActor ? "" : "none";
  if (navStoryteller)
    navStoryteller.style.display = fv.viewStoryteller ? "" : "none";
  if (navVoiceProduction)
    navVoiceProduction.style.display = fv.viewVoiceProduction ? "" : "none";
  if (navUtilities) navUtilities.style.display = fv.viewUtility ? "" : "none";

  // Tab links (Voice Actor)
  const tabLineReader = document.querySelector(
    'button[data-tab="scripts"]',
  ) as HTMLElement;
  const tabTeleprompt = document.querySelector(
    'button[data-tab="teleprompt"]',
  ) as HTMLElement;
  const tabAuditions = document.querySelector(
    'button[data-tab="auditions"]',
  ) as HTMLElement;
  const tabEffectLibrary = document.querySelector(
    'button[data-tab="effect-library"]',
  ) as HTMLElement;

  if (tabLineReader)
    tabLineReader.style.display = fv.tabLineReader ? "" : "none";
  if (tabTeleprompt)
    tabTeleprompt.style.display = fv.tabTeleprompter ? "" : "none";
  if (tabAuditions) tabAuditions.style.display = fv.tabAuditions ? "" : "none";
  if (tabEffectLibrary)
    tabEffectLibrary.style.display = fv.tabEffectLibrary ? "" : "none";

  // Tab links (Voice Production)
  const tabVpFeedback = document.querySelector(
    'button[data-tab="vp-feedback"]',
  ) as HTMLElement;
  const tabVpScript = document.querySelector(
    'button[data-tab="vp-script"]',
  ) as HTMLElement;
  const tabVpSides = document.querySelector(
    'button[data-tab="vp-sides"]',
  ) as HTMLElement;
  const tabVpAuditions = document.querySelector(
    'button[data-tab="vp-auditions"]',
  ) as HTMLElement;
  const tabVpContraster = document.querySelector(
    'button[data-tab="vp-contraster"]',
  ) as HTMLElement;
  const tabVpTableRead = document.querySelector(
    'button[data-tab="vp-table-read"]',
  ) as HTMLElement;

  if (tabVpFeedback)
    tabVpFeedback.style.display = fv.tabVpFeedback ? "" : "none";
  if (tabVpScript) tabVpScript.style.display = fv.tabVpScript ? "" : "none";
  if (tabVpSides) tabVpSides.style.display = fv.tabVpSides ? "" : "none";
  if (tabVpAuditions)
    tabVpAuditions.style.display = fv.tabVpAuditions ? "" : "none";
  if (tabVpContraster)
    tabVpContraster.style.display = fv.tabVpContraster ? "" : "none";
  if (tabVpTableRead)
    tabVpTableRead.style.display = fv.tabVpTableRead ? "" : "none";

  // Tab links (Storyteller)
  const tabDmSession = document.querySelector(
    'button[data-tab="dm-session"]',
  ) as HTMLElement;
  const tabDmGenerator = document.querySelector(
    'button[data-tab="dm-generator"]',
  ) as HTMLElement;
  const tabDmCharacterNotes = document.querySelector(
    'button[data-tab="dm-character-notes"]',
  ) as HTMLElement;

  if (tabDmSession) tabDmSession.style.display = fv.tabDmSession ? "" : "none";
  if (tabDmGenerator)
    tabDmGenerator.style.display = fv.tabDmGenerator ? "" : "none";
  if (tabDmCharacterNotes)
    tabDmCharacterNotes.style.display = fv.tabDmCharacterNotes ? "" : "none";

  // Tab links (Utility)
  const tabWarmups = document.querySelector(
    'button[data-tab="warm-ups"]',
  ) as HTMLElement;
  const tabVoiceMemos = document.querySelector(
    'button[data-tab="voice-memos"]',
  ) as HTMLElement;
  const tabAudioOverlay = document.querySelector(
    'button[data-tab="audio-overlay"]',
  ) as HTMLElement;

  if (tabWarmups) tabWarmups.style.display = fv.tabWarmups ? "" : "none";
  if (tabVoiceMemos)
    tabVoiceMemos.style.display = fv.tabVoiceMemos ? "" : "none";
  if (tabAudioOverlay)
    tabAudioOverlay.style.display = fv.tabAudioOverlay ? "" : "none";

  // Fallback logic for active elements that are now hidden

  // Fallback Views
  const activeNavLinks = Array.from(
  ...
  // Fallback Tabs
  const activeTabLinks = Array.from(
    document.querySelectorAll(".tab-link.active"),
  ) as HTMLElement[];
    ...
    if (tab.style.display === "none") {
      ...
      if (parent) {
        const visibleTabs = Array.from(
          parent.querySelectorAll(".tab-link"),
